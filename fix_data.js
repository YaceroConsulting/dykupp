import fs from 'fs';
import data from './app/expositionstid.json' assert { type: 'json' };
...
// Since there's `new Date()`, we can just replace that string to something eval-able
...
result.forEach((dive, i) => {
    ...
    if (dive.maxRemaining.diveTimeAtDepth !== undefined) {
        ...
        // non-max question
        // if maxRemaining was provided, calculate it correctly? Actually, it's just `maxRemaining` field sometimes.
...
// the inspect outputs Date objects as 2022-01... let's replace them back to `startTime`
